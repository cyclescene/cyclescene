...
Use the provided migration script to initialize the monitoring database schema:

```bash
./migrate-monitoring.sh
```

This will create the `sync_logs` table and all necessary indexes.

**Alternative: Manual Migration**

If you prefer to run migrations manually via Turso console:

```bash
turso db shell cycle-scene-monitoring
```

Then execute the SQL from `db/migrations/monitoring/1734782400_create_sync_logs_table.up.sql`
