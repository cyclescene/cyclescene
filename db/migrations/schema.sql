...
CREATE TABLE geocode_cache (location_key TEXT PRIMARY KEY, lat REAL NOT NULL, lng REAL NOT NULL, last_updated TEXT NOT NULL);
CREATE UNIQUE INDEX idx_geocode_key ON geocode_cache (location_key);
CREATE TABLE events (id INTEGER PRIMARY KEY, title TEXT NOT NULL, tinytitle TEXT, description TEXT NOT NULL, image_url TEXT, audience TEXT, ride_length TEXT, area TEXT, date_type TEXT, venue_name TEXT, address TEXT, location_details TEXT, ending_location TEXT, is_loop_ride INTEGER NOT NULL DEFAULT 0, organizer_name TEXT, organizer_email TEXT, organizer_phone TEXT, web_url TEXT, web_name TEXT, newsflash TEXT, hide_email INTEGER NOT NULL DEFAULT 0, hide_phone INTEGER NOT NULL DEFAULT 0, hide_contact_name INTEGER NOT NULL DEFAULT 0, group_code TEXT, edit_token TEXT UNIQUE, is_published INTEGER NOT NULL DEFAULT 0, is_featured INTEGER NOT NULL DEFAULT 0, created_at TEXT NOT NULL DEFAULT (STRFTIME ('%Y-%m-%d %H:%M:%f', 'NOW')), updated_at TEXT NOT NULL DEFAULT (STRFTIME ('%Y-%m-%d %H:%M:%f', 'NOW')), FOREIGN KEY (group_code) REFERENCES GROUPS (code) ON DELETE SET NULL);
CREATE INDEX idx_published ON events (is_published);
CREATE INDEX idx_group_code ON events (group_code);
CREATE TABLE event_occurrences (id INTEGER PRIMARY KEY, event_id INTEGER NOT NULL, start_date TEXT NOT NULL, start_time TEXT NOT NULL, start_datetime TEXT NOT NULL, event_duration_minutes INTEGER, event_time_details TEXT, is_cancelled INTEGER NOT NULL DEFAULT 0, FOREIGN KEY (event_id) REFERENCES events (id) ON DELETE CASCADE);
