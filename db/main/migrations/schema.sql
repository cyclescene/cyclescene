...
CREATE TABLE strava_event_metadata (event_id INTEGER PRIMARY KEY, strava_event_id INTEGER NOT NULL, strava_club_id INTEGER NOT NULL, imported_by_athlete_id INTEGER NOT NULL, last_refreshed_at TEXT NOT NULL DEFAULT (STRFTIME ('%Y-%m-%d %H:%M:%f', 'NOW')), FOREIGN KEY (event_id) REFERENCES events (id) ON DELETE CASCADE, FOREIGN KEY (imported_by_athlete_id) REFERENCES strava_connections (athlete_id) ON DELETE CASCADE);
