...
CREATE TABLE "ride_groups" (id TEXT PRIMARY KEY, code TEXT UNIQUE NOT NULL, name TEXT NOT NULL, description TEXT, city TEXT, icon_url TEXT, is_active INTEGER NOT NULL DEFAULT 1, web_url TEXT, edit_token TEXT UNIQUE, created_at TEXT NOT NULL, public_id TEXT UNIQUE, marker TEXT, marker_color TEXT DEFAULT '#3B82F6', email TEXT);
...
CREATE TABLE "events" (id INTEGER PRIMARY KEY, title TEXT NOT NULL, tinytitle TEXT, description TEXT NOT NULL, image_url TEXT, audience TEXT, ride_length TEXT, area TEXT, date_type TEXT, venue_name TEXT, address TEXT, location_details TEXT, ending_location TEXT, is_loop_ride INTEGER NOT NULL DEFAULT 0, city TEXT NOT NULL, organizer_name TEXT, organizer_email TEXT, organizer_phone TEXT, web_url TEXT, web_name TEXT, newsflash TEXT, hide_email INTEGER NOT NULL DEFAULT 0, hide_phone INTEGER NOT NULL DEFAULT 0, hide_contact_name INTEGER NOT NULL DEFAULT 0, group_code TEXT, group_id TEXT, edit_token TEXT UNIQUE, is_published INTEGER NOT NULL DEFAULT 0, is_featured INTEGER NOT NULL DEFAULT 0, moderation_notes TEXT, moderated_at TEXT, created_at TEXT NOT NULL DEFAULT (STRFTIME ('%Y-%m-%d %H:%M:%f', 'NOW')), updated_at TEXT NOT NULL DEFAULT (STRFTIME ('%Y-%m-%d %H:%M:%f', 'NOW')), latitude REAL, longitude REAL, image_uuid TEXT, route_id TEXT REFERENCES routes (id) ON DELETE SET NULL, source TEXT, source_id TEXT, FOREIGN KEY (group_id) REFERENCES ride_groups (id) ON DELETE SET NULL);
...
CREATE INDEX idx_groups_email ON ride_groups (email);
CREATE TABLE routes (id TEXT PRIMARY KEY, source TEXT NOT NULL, source_id TEXT NOT NULL, source_url TEXT NOT NULL, geojson TEXT NOT NULL, distance_km REAL, distance_mi REAL, created_at TEXT NOT NULL DEFAULT (STRFTIME ('%Y-%m-%d %H:%M:%f', 'NOW')), city TEXT DEFAULT 'pdx', UNIQUE (source, source_id));
...
CREATE TABLE strava_connections (athlete_id INTEGER PRIMARY KEY, refresh_token_encrypted BLOB NOT NULL, encryption_nonce BLOB NOT NULL, city_code TEXT NOT NULL, last_synced_at TEXT, created_at TEXT NOT NULL DEFAULT (STRFTIME ('%Y-%m-%d %H:%M:%f', 'NOW')));
CREATE INDEX idx_strava_connections_city ON strava_connections (city_code);
CREATE INDEX idx_strava_connections_last_synced ON strava_connections (last_synced_at);
CREATE TABLE strava_event_metadata (event_id INTEGER PRIMARY KEY, strava_event_id INTEGER NOT NULL, strava_club_id INTEGER NOT NULL, imported_by_athlete_id INTEGER NOT NULL, last_refreshed_at TEXT NOT NULL DEFAULT (STRFTIME ('%Y-%m-%d %H:%M:%f', 'NOW')), imported_at TEXT NOT NULL DEFAULT (STRFTIME ('%Y-%m-%d %H:%M:%f', 'NOW')), refresh_count INTEGER NOT NULL DEFAULT 0, FOREIGN KEY (event_id) REFERENCES events (id) ON DELETE CASCADE, FOREIGN KEY (imported_by_athlete_id) REFERENCES strava_connections (athlete_id) ON DELETE CASCADE);
CREATE INDEX idx_strava_event_metadata_strava_ids ON strava_event_metadata (strava_club_id, strava_event_id);
CREATE INDEX idx_strava_event_metadata_athlete ON strava_event_metadata (imported_by_athlete_id);
CREATE INDEX idx_strava_event_metadata_refreshed ON strava_event_metadata (last_refreshed_at);
CREATE INDEX idx_strava_event_metadata_imported_at ON strava_event_metadata (imported_at);